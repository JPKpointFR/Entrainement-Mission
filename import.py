...
def generate_json_file(categorie, titre, url):
    ...
    try:
        ...
        print(f"Exception pour la requete HTTP GET sur: {url}")
    else:
        try:
            data = json.loads(response.text)
            all_quizz = data["quizz"]["fr"]
            for quizz_title, quizz_data in all_quizz.items():
                out_filename = get_quizz_filename(
                    categorie, titre, quizz_title)
                print(out_filename)
            ...
            print(
                f"Exception dans la déserialisation où l'utilisation des données: Questionnaire: {titre} - url: {url}")


for quizz_data in open_quizz_db_data:
    questionnaire = {generate_json_file(
        quizz_data[0], quizz_data[1], quizz_data[2])}
